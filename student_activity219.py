...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        position = value_list.index(detection.object_tag) #stores value in object_id
        distance= math.sqrt((detection.point.x ** 2) + (detection.point.y ** 2) + (detection.point.z ** 2))
        distance_name_data["distance"] = distance #distance = key value= whatevr the distance is from the car (distance_name_data)
        distance_name_data["name"] = key_list[position] #key= name value = e.g Roads
...
try:
    ...
    spawn_point = (world.get_map().get_spawn_points()[20]) #spawn point changes from last class
